...
function validate_file_upload($file) {
    $errors = [];
    if ($file['error'] !== UPLOAD_ERR_OK) { $errors[] = 'File upload error occurred.'; return $errors; }
    if ($file['size'] > MAX_FILE_SIZE)     { $errors[] = 'File size exceeds maximum allowed size.'; }
    $extension = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    if (!in_array($extension, ALLOWED_FILE_TYPES)) { $errors[] = 'File type not allowed.'; }
    return $errors;
}
...
function user_avatar_url($user) {
    ...
    if (!empty($user['photo']) && file_exists($user['photo'])) return $user['photo'];
    ...
    $keys = $gender_file_map[$gender] ?? $gender_file_map['male'];
    foreach ($bases as $base) {
...
function get_base_url() { return rtrim(BASE_URL, '/') . '/'; }

function generate_random_password($length = 12) {
    $chars    = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+';
    $password = '';
    $len      = strlen($chars);
    for ($i = 0; $i < $length; $i++) $password .= $chars[random_int(0, $len - 1)];
    return $password;
}

// ============================================================
// Auto-load the auth helper so can() is always available
// ============================================================
